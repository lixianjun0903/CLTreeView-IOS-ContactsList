...
- (void)viewDidLoad
{
    ...
    CGRect tableViewFrame = CGRectMake(0, 25, self.view.frame.size.width, self.view.frame.size.height-70);
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(node.type == 2){
        //处理叶子节点选中，此处需要自定义
        lixianjunViewController * lixanjun = [[lixianjunViewController alloc] init];
        CLTreeView_LEVEL2_Cell * cell = (CLTreeView_LEVEL2_Cell *)[tableView cellForRowAtIndexPath:indexPath];
        lixanjun.str = cell.signture.text;
        UINavigationController * nav = [[UINavigationController alloc] initWithRootViewController:lixanjun];
